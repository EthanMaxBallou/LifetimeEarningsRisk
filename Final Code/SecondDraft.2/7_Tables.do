...
global OUTDIR  "$REPODIR/OtherOutput/SecondDraft2"
...
foreach outn in gamma alpha {
    ...
    foreach meas in fhwage fearn {
        ...
        foreach meth in nn rf lasso {
            ...
            preserve
                import delimited using "$REPODIR/`pre'shap_summary_`outn'`sfx'.csv", clear
        ...
        foreach meas in fhwage fearn {
            ...
            * the NN files, whose codes are already numeric)
            foreach mod in lasso nn rf {
                ...
                preserve
                    import delimited using "$REPODIR/`pre'`csvstem'_shap_`outn'`sfx'.csv", clear
                    rename `codevar' code
