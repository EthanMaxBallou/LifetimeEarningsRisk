...
nn1.save(os.path.join(MODELDIR, "Risk_NN_Gamma.keras"))
...
# Separate the target variable from the data
target = data[['gammaP_WEIGHTED_fearn']]  # Extract the target column
data = data.drop(columns=['gammaP_WEIGHTED_fearn'], errors='ignore')  # Remove the target column from the data
...
nn1.save(os.path.join(MODELDIR, "Risk_NN_Gamma_fearn.keras"))
